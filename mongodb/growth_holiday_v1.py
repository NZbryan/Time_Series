...
def inert_mongo(k):
    ...
    generate_dict = {'names':names,
                     ...
                     '节假日代码':list(range(1,11)),
                     '节假日类型': ['春节','清明节','五一劳动节','端午节','六一儿童节',
                               '十一国庆节','618购物节','双十一购物节','圣诞节','元旦'],
                     '成正比例_同比_整体': np.around(np.nanmean(TB_data, axis=1),decimals=2).tolist(),
                     '成正比例_同比':TB_data,
                     '成正比例_环比':HB_data,
                     '成正比例_环比_整体': np.around(np.nanmean(HB_data, axis=1),decimals=2).tolist(),
                     }
...
if __name__ == '__main__':
    ...
    pool.map(inert_mongo, length_list)  # process data_inputs iterable with pool