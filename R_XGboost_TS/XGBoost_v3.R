...
for(j in seq(nrow(alldata))){
  ...
  vl = match(predict_7day,row1_existdate)
...
predict_7day_df = as.data.frame(predict_7day_df,row.names = train_set_3$names)
...
forecast_7 = foreach(x=seq(length(loop_feature)),
